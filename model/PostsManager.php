...
class PostsManager {
    ...
    function getPosts($nb_billet=10) {
        $db = Connection::getInstance();
        $req = $db->query('SELECT id, title, content, DATE_FORMAT(creation_date, \'%d/%m/%Y &agrave %H:%i\') AS creation_date_fr FROM posts ORDER BY creation_date ASC LIMIT 0,'.$nb_billet);
        return $req;
    }
